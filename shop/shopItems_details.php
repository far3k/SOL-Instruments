...
# Single product page, wishlist toggle, add to shop cart.
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["add_to_wishlist"])) {
    ...
    header("Location: " . sol_url("shop/shopItems_details.php?id=" . $productId));
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["add_to_cart"])) {
    if (sol_csrf_verify()) {
        ...
        if ($cartId === $productId && $cartId > 0) {
            $_SESSION["shop_cart"][$cartId] = ($_SESSION["shop_cart"][$cartId] ?? 0) + 1;
        }
    }
    header("Location: " . sol_url("shop/shopItems_details.php?id=" . $productId));
    exit;
...
$st = $connect->prepare("SELECT * FROM products WHERE id = ? LIMIT 1");
...
$inWishlist = false;
if ($uid > 0) {
    $w = $connect->prepare("SELECT 1 FROM wishlist WHERE user_id = ? AND product_id = ? AND item_type = 'product' LIMIT 1");
...
$page_title = ($row["name"] ?? "Product") . " — Details";
$nav_role = "user";
    ...
    <div class="card sol-card-shell border-0 shadow-sm overflow-hidden sol-product-detail-card">
        <div class="row g-0">
            <div class="col-md-6 d-flex align-items-center justify-content-center p-3 sol-product-detail-media" style="min-height: 280px;">
                <img src="<?= htmlspecialchars(sol_url("pictures/" . $pic), ENT_QUOTES, "UTF-8") ?>" class="img-fluid sol-product-detail-img" style="max-height: 420px; object-fit: contain;" alt="<?= $name ?>">
            </div>
            <div class="col-md-6 sol-product-detail-content-col">
                <div class="card-body p-4">
                    <h1 class="h3 mb-3"><?= $name ?></h1>
                    <p class="fs-4 text-primary mb-4">€<?= $price ?></p>
                    <div class="mb-4"><?= $desc ?></div>

                    <div class="d-flex flex-wrap gap-2 mb-3">
                        <?php if (!$inWishlist): ?>
                            <div class="sol-wishlist-add-wrap d-inline">
                                <form method="post" class="d-inline sol-ajax-cart" data-sol-wish-type="product">
                                    <?= sol_csrf_field() ?>
                        ...
                        <form method="post" class="d-inline sol-ajax-cart">
                            <?= sol_csrf_field() ?>
                            <input type="hidden" name="product_id" value="<?= (int)$productId ?>">
                            <button type="submit" name="add_to_cart" class="btn btn-outline-success btn-sm">Add to shop cart</button>
                        </form>
                    </div>

                    <a href="<?= htmlspecialchars(sol_url("shop/catalog.php"), ENT_QUOTES, "UTF-8") ?>" class="btn btn-secondary">Back to accessories</a>
                </div>
            </div>
        </div>
    </div>
